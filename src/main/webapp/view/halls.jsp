...
<div class="exhibitions_container">
  <c:if test="${hallsToShow != null}">
    ...
    <table align="center" class="exhibitions" cellspacing="9">
      <c:forEach var="hall" items="${hallsToShow}">
  ...
  <div class="pagination">
    <div class="pagination-inner">
      <c:forEach var="i" begin="1" end="${numberOfPagesHalls}">
        <c:choose>
          <c:when test="${currentPageHalls == i}">
            <a class="active" href="home?command=getHalls&pageNum=${i}">${i}</a>
          </c:when>
          <c:otherwise>
            <a href="home?command=getHalls&pageNum=${i}">${i}</a>
          </c:otherwise>
        </c:choose>
      </c:forEach>
    </div>
